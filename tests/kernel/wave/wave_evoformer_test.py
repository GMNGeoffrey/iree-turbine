...
def testEvoformerAttentionForward_bf16(
    shape: tuple[int],
    enable_scheduling: bool,
    mfma_variant: MMAType,
    request,
    ...
    @tkw.wave(constraints)
    def evoformer_fwd(
        q: tkl.Memory[B, BN, M, H, K1, ADDRESS_SPACE, tkl.bf16],
        k: tkl.Memory[B, BN, K2, H, K1, ADDRESS_SPACE, tkl.bf16],
        v: tkl.Memory[B, BN, N, H, K2, ADDRESS_SPACE, tkl.bf16],
        mask: tkl.Memory[B, BN, K2, GLOBAL_ADDRESS_SPACE, tkl.bf16],
        bias: tkl.Memory[B, H, M, K2, GLOBAL_ADDRESS_SPACE, tkl.bf16],
        c: tkl.Memory[B, BN, M, H, N, GLOBAL_ADDRESS_SPACE, tkl.bf16],
    ):
        c_reg = tkl.Register[B, BN, H, N, M, tkl.f32](0.0)
        init_sum = tkl.Register[B, BN, H, M, tkl.f32](0.0)
        init_max = tkl.Register[B, BN, H, M, tkl.f32](-1e6)
    ...
    with tk.gen.TestLaunchContext(
        ...
        batch, n, kv_seq_len, heads, head_dim, q_seq_len, v_dim = shape
        # q: tkl.Memory[B, BN, M, H, K1, ADDRESS_SPACE, tkl.bf16],
        q = device_randn(batch, n, q_seq_len, heads, head_dim, dtype=torch.bfloat16)
        # k: tkl.Memory[B, BN, K2, H, K1, ADDRESS_SPACE, tkl.bf16],
        k = device_randn(batch, n, kv_seq_len, heads, head_dim, dtype=torch.bfloat16)
        # v: tkl.Memory[B, BN, K2, H, N, ADDRESS_SPACE, tkl.bf16],
        v = device_randn(batch, n, kv_seq_len, heads, v_dim, dtype=torch.bfloat16)
        # mask: tkl.Memory[B, BN, K2, GLOBAL_ADDRESS_SPACE, tkl.bf16],
        mask = device_randint(0, 2, (batch, n, kv_seq_len), dtype=torch.bfloat16)
        mask_bias = 1e9 * (mask - 1)
        # bias: tkl.Memory[B, H, M, K2, GLOBAL_ADDRESS_SPACE, tkl.bf16],
        bias = device_randn(batch, heads, q_seq_len, kv_seq_len, dtype=torch.bfloat16)
        # output: tkl.Memory[B, BN, M, H, N, GLOBAL_ADDRESS_SPACE, tkl.bf16],
        output = device_zeros(batch, n, q_seq_len, heads, v_dim, dtype=torch.bfloat16)
        log2e = 1.44269504089
        dk_sqrt = math.sqrt(1.0 / shape[4])
        # TODO: Add scaling of QK as part of kernel.
        mb = evoformer_fwd(

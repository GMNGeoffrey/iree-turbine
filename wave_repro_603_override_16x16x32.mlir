...
module attributes {transform.with_named_sequence} {
  stream.executable private @repro_603 {
    ...
    builtin.module {
      func.func @repro_603(%arg_a: !stream.binding, %arg_b: !stream.binding, %arg_e: !stream.binding, %arg_a_transpose: !stream.binding, %arg_c: !stream.binding, %arg_d: !stream.binding) attributes {translation_info = #translation} {
        ...
        %a = stream.binding.subspan %arg_a[%c0] : !stream.binding -> memref<32x16xf16, strided<[16, 1], offset: ?>>
        %b = stream.binding.subspan %arg_b[%c0] : !stream.binding -> memref<16x16xf16, strided<[16, 1], offset: ?>>
        %c = stream.binding.subspan %arg_c[%c0] : !stream.binding -> memref<32x16xf32, strided<[16, 1], offset: ?>>
        %e = stream.binding.subspan %arg_e[%c0] : !stream.binding -> memref<16x32xf16, strided<[32, 1], offset: ?>>
        %a_transpose = stream.binding.subspan %arg_a_transpose[%c0] : !stream.binding -> memref<16x32xf16, strided<[32, 1], offset: ?>>
        ...
        %34 = scf.for %arg6 = %c0 to %c2 step %c1 iter_args(%arg7 = %c0_4vf32) -> (vector<4xf32>) {
          %46 = arith.muli %arg6, %c16 overflow<nsw, nuw> : index
          %47 = arith.addi %2, %46 overflow<nsw, nuw> : index
          %a_reg = vector.load %a[%47, %8] : memref<32x16xf16, strided<[16, 1], offset: ?>>, vector<4xf16>
          ...
          %51 = arith.addi %46, %6 overflow<nsw, nuw> : index
          vector.store %50, %c[%51, %3] : memref<32x16xf32, strided<[16, 1], offset: ?>>, vector<1xf32>
          %52 = vector.extract_strided_slice %49 {offsets = [1], sizes = [1], strides = [1]} : vector<4xf32> to vector<1xf32>
          %53 = arith.addi %51, %c1 overflow<nsw, nuw> : index
          vector.store %52, %c[%53, %3] : memref<32x16xf32, strided<[16, 1], offset: ?>>, vector<1xf32>
          %54 = vector.extract_strided_slice %49 {offsets = [2], sizes = [1], strides = [1]} : vector<4xf32> to vector<1xf32>
          %55 = arith.addi %51, %c2 overflow<nsw, nuw> : index
          vector.store %54, %c[%55, %3] : memref<32x16xf32, strided<[16, 1], offset: ?>>, vector<1xf32>
          %56 = vector.extract_strided_slice %49 {offsets = [3], sizes = [1], strides = [1]} : vector<4xf32> to vector<1xf32>
          %57 = arith.addi %51, %c3 overflow<nsw, nuw> : index
          vector.store %56, %c[%57, %3] : memref<32x16xf32, strided<[16, 1], offset: ?>>, vector<1xf32>
          %e_reg = vector.load %e[%3, %51] : memref<16x32xf16, strided<[32, 1], offset: ?>>, vector<4xf16>
  ...
  func.func @isolated_benchmark(%arg_a: tensor<32x16xf16>, %arg_b: tensor<16x16xf16>, %arg_e: tensor<16x32xf16>, %arg_a_transpose: tensor<16x32xf16>, %arg_c: tensor<32x16xf32>, %arg_d: tensor<16x16xf16>) -> (tensor<16x32xf16>, tensor<32x16xf32>, tensor<16x16xf16>) {
    %b:3 = flow.dispatch @repro_603::@repro_603(%arg_a, %arg_b, %arg_e, %arg_a_transpose, %arg_c, %arg_d) : (tensor<32x16xf16>, tensor<16x16xf16>, tensor<16x32xf16>, tensor<16x32xf16>, tensor<32x16xf32>, tensor<16x16xf16>) -> (%arg_a_transpose, %arg_c, %arg_d)
    return %b#0, %b#1, %b#2 : tensor<16x32xf16>, tensor<32x16xf32>, tensor<16x16xf16>
  }
}

  ...
  stream.executable private @repro_603 {
    ...
    builtin.module {
      func.func @repro_603(%arg_a: !stream.binding, %arg_b: !stream.binding, %arg_e: !stream.binding, %arg_a_transpose: !stream.binding, %arg_c: !stream.binding, %arg_d: !stream.binding) attributes {translation_info = #translation} {
        ...
        %b = stream.binding.subspan %arg_b[%c0] : !stream.binding -> memref<16x32xf16, strided<[32, 1], offset: ?>>
        ...
        %b_reg = vector.load %b[%3, %8] : memref<16x32xf16, strided<[32, 1], offset: ?>>, vector<4xf16>
        %a = stream.binding.subspan %arg_a[%c0] : !stream.binding -> memref<16x32xf16, strided<[32, 1], offset: ?>>
        ...
        %a_transpose = stream.binding.subspan %arg_a_transpose[%c0] : !stream.binding -> memref<32x16xf16, strided<[16, 1], offset: ?>>
        %a_reg = vector.load %a[%2, %8] : memref<16x32xf16, strided<[32, 1], offset: ?>>, vector<4xf16>
        ...
        %x_div_ept = arith.divsi %x_mod_tpw, %els_per_thread : index // (x // els_per_thread)
        %x_div_ept_times_16 = arith.muli %x_div_ept, %c16 : index // (x // els_per_thread) * 16
        %x_div_16 = arith.divsi %x_mod_tpw, %c16 : index        // (x // 16)
        %x_div_16_times_ept = arith.muli %els_per_thread, %x_div_16 : index  // els_per_thread * (x // 16)
        %x_plus_ept = arith.addi %x_mod_tpw, %els_per_thread : index

        // unrolled loop (4)
        // shuff_i = 0
        %shuff0 = arith.constant 0 : index
        %x_plus_shuff0 = arith.addi %x_mod_tpw, %shuff0 : index  // x + shuff0
        ...
        %x_plus_shuff1 = arith.addi %x_mod_tpw, %shuff1 : index
        ...
        %x_plus_shuff2 = arith.addi %x_mod_tpw, %shuff2 : index
        ...
        %x_plus_shuff3 = arith.addi %x_mod_tpw, %shuff3 : index
        ...
        %n_idx = arith.addi %2, %7 : index
        vector.store %a_t_reg, %a_transpose[%n_idx, %6] : memref<32x16xf16, strided<[16, 1], offset: ?>>, vector<4xf16>
        ...
        %d = stream.binding.subspan %arg_d[%c0] : !stream.binding -> memref<16x32xf16, strided<[32, 1], offset: ?>>
        %28 = arith.addi %1, %6 overflow<nsw, nuw> : index
        %29 = arith.addi %2, %7 overflow<nsw, nuw> : index
        vector.store %26, %d[%28, %29] : memref<16x32xf16, strided<[32, 1], offset: ?>>, vector<1xf16>
        ...
        vector.store %30, %d[%31, %29] : memref<16x32xf16, strided<[32, 1], offset: ?>>, vector<1xf16>
        ...
        vector.store %32, %d[%33, %29] : memref<16x32xf16, strided<[32, 1], offset: ?>>, vector<1xf16>
        ...
        vector.store %34, %d[%35, %29] : memref<16x32xf16, strided<[32, 1], offset: ?>>, vector<1xf16>
        return
      }
    }
  }
  func.func @isolated_benchmark(%arg_a: tensor<16x32xf16>, %arg_b: tensor<16x32xf16>, %arg_e: tensor<16x16xf16>, %arg_a_transpose: tensor<32x16xf16>, %arg_c: tensor<16x16xf32>, %arg_d: tensor<16x32xf16>) -> (tensor<32x16xf16>, tensor<16x16xf32>, tensor<16x32xf16>) {
    %b:3 = flow.dispatch @repro_603::@repro_603(%arg_a, %arg_b, %arg_e, %arg_a_transpose, %arg_c, %arg_d) : (tensor<16x32xf16>, tensor<16x32xf16>, tensor<16x16xf16>, tensor<32x16xf16>, tensor<16x16xf32>, tensor<16x32xf16>) -> (%arg_a_transpose, %arg_c, %arg_d)
    return %b#0, %b#1, %b#2 : tensor<32x16xf16>, tensor<16x16xf32>, tensor<16x32xf16>
  }
}
